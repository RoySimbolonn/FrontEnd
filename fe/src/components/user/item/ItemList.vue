...
export default {
  ...
  data() {
    return {
      ...
      showForm: false,
      selectedItem: null,
      searchQuery: "", // Menambahkan searchQuery untuk menangani pencarian
    };
  },

  computed: {
    // Properti terkomputasi untuk menyaring item berdasarkan searchQuery
    filteredItems() {
      if (this.searchQuery) {
  ...
  methods: {
    ...
    handleBorrow(item) {
      ...
      // Implementasi logika peminjaman barang
      this.showForm = false;
    },
    ...
    // Metode untuk menangani pencarian
    handleSearch(query) {
  ...
  mounted() {
    // Mendaftarkan event listener untuk 'search'
